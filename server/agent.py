...
async def install_skill(slug: str) -> str:
    ...
    entry = next((s for s in cat["skills"] if s.get("slug") == slug or s.get("name") == slug), None)
    if not entry:
        raise ValueError(f"{slug} not in catalog")
    ...
    async with httpx.AsyncClient(timeout=120, follow_redirects=True) as c:
        blob = (await c.get(tar_url)).content
    dest = SKILLS_DIR / Path(entry["path"]).name
    ...
    with tarfile.open(fileobj=io.BytesIO(blob), mode="r:gz") as tf:
        for mem in tf.getmembers():
            if mem.name.startswith(prefix) and mem.isfile():
                ...
                out.write_bytes(tf.extractfile(mem).read())
    return str(dest)
